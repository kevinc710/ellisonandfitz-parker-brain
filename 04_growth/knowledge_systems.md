# Internal Knowledge Systems

## "Second Brain" (coaching content transcription)

Kevin is building an AI-powered "Second Brain" by transcribing coaching
program content — **EcomTalent Creative** and **Evolve Blueprint** (13
modules) — via Whisper.

- Stored in Google Drive: `02_Evolve_Blueprint transcripts`
  (folder ID `1dJaK_bD2UfmaNg3N_N9h4eVGJ36lxlGy`)
- Also stored in a private GitHub repo:
  `kevinc710/evolve-blueprint-transcripts`
- Abigail handles transcription/uploads

## SOP hub

`kevinc710.github.io/ef-sops/` — single `index.html` file, embedded in
Notion. Claude cannot push to it directly without a GitHub PAT.

## This E&F Brand Brain repo

This repo (`ellisonandfitz-parker-brain`) is a separate,
purpose-built knowledge base for the E&F agent — distinct from the Second Brain
transcripts and the SOP hub above. It should stay focused on structured
brand/operations knowledge rather than raw transcripts.
